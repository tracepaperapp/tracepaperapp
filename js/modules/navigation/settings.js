document.addEventListener('alpine:init', () => {
    Alpine.data('projectSettings', function(){
        return {
            drn: this.$persist("").using(sessionStorage),
            settings: this.$persist({}).using(sessionStorage),
            config: {},
            async init(){
                let api = await API.initialize(true);
                let data = await api.query("/prepared-statements/get-project.txt",{key:this.drn},true);
                this.settings = data.data.Project.get;
                this.config = await Modeler.get_model("config.xml");
                this.$watch("config",this.save.bind(this));
            },
            async save(){
                await Modeler.save_model("config.xml",this.config);
            },
            create_pipeline(region){
                let repo = this.settings.repositories.filter(x => x.name == 'code').at(0).url;
                let url = 'https://' + region.code + '.console.aws.amazon.com/cloudformation/home?region='
                    ...
                    + this.settings.name.toLowerCase() + '-main-pipeline&param_GithubWorkspace='
                    ...
                    + this.settings.name.toLowerCase() + '&param_DRN='
                    + this.drn + '&param_GraphQL=' + api_url + '&param_APIKEY=' + api_key;
