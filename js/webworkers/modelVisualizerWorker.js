    ...
    try {
        switch (action) {
          case 'node-diagram':
            ...
            let data = await node_diagram(file);
            postMessage({ result: data, request_id });
            break;
          default:
...
async function node_diagram(file){
    files = await isogit.listFiles({ fs, dir: dir, ref: 'HEAD' });
    return await Diagram.draw(file);
}
...
Diagram = {
    ...
    prepare_translations: async function(file){
        ...
        let items = [...files];
        items = items.filter(x => x.endsWith(".xml"));
        ...
        for (let i = 0; i < items.length; i++){
            ...
            if (type == "behavior" && !aggregate){
                let model = await get_model(source,true);
    ...
    prepare_diagram_data: async function(file){
        ...
        items = files.filter(x => x.endsWith(".xml"));
        let aggregate = file.split(";").at(0);
        aggregate = !(aggregate.startsWith("domain/") && aggregate.split("/").filter(x => x != '').length > 2);
        for (let i = 0; i < items.length; i++){
            let source = items[i];
...
//                let scenario = await get_model(files[i],true);
